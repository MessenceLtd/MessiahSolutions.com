  ...
  .controller("UmbracoForms.Dashboards.FormsController",
    function ($scope, $location, $cookies, formResource, licensingResource, updatesResource, notificationsService, userService, securityResource, recordResource, localizationService) {
      ...
      userService.getCurrentUser().then(function (response) {
        ...
        securityResource.getByUserId(vm.currentUser.id, false).then(function (response) {
...
angular.module("umbraco").controller("UmbracoForms.Editors.DataSource.EditController",
  function ($scope, $routeParams, dataSourceResource, editorState, notificationsService, editorService, navigationService, formHelper, userService, securityResource, localizationService, providerLocalizationHelper) {
    ...
    userService.getCurrentUser().then(function (response) {
      ...
      securityResource.getByUserId(currentUserId, false).then(function (response) {
...
(function () {
  ...
  function formDesignController(formResource, userService, securityResource, localizationService, providerLocalizationHelper) {
    ...
    userService.getCurrentUser().then(function (response) {
      ...
      //Now we can make a call to form securityResource
      securityResource.getByUserId(currentUser.id, false).then(function (response) {
...
(function () {
  ...
  function FormSecurityEditController($scope, $routeParams, securityResource, notificationsService, navigationService, localizationService, editorService, usersResource, userGroupsResource) {
    ...
    // This view/controller is used for editing groups, users or start folders, which we can distinguish by the prefix
    // to the id provided in the route parameters.
    // We also parse the actual id without the prefix.
    var prefixes = ["group", "user", "start-folder"];
    ...
    // When user group editing is configured, start folders are editing on their own page (this is necessary, as we need to
    // have a record per user available for editing; whereas for the other details, these are only created when needed for
    // user specific overrides to the group permissions).
    // When permissions are managed only on users, we can edit them on the same page, as there will be a record and a tree
    // entry available for all users.
    // Forms in folders are also only supported when form definitions are stored in the database.
    vm.startFoldersSupported = Umbraco.Sys.ServerVariables.umbracoPlugins.forms.storeUmbracoFormsInDb &&
      (vm.editing == "start-folder" ||
        (vm.editing == "user" && !vm.userGroupEditingSupported));
    ...
    function save() {
      ...
      localizationService.localizeMany(["formSecurity_saveSuccessTitle", "formSecurity_saveErrorTitle"]).then(function (labels) {
        ...
        if (vm.editing == "group") {
          ...
          if (vm.editing == "start-folder") {
            vm.security.saveOption = "StartFolders";  // Editing start folders for a user, so only this information should be updated.
          } else if (vm.userGroupEditingSupported) {
            vm.security.saveOption = "Security";      // Editing security for a user, without start folders, so only update security info.
          } else {
            vm.security.saveOption = "All";           // Editing security and start folders together, so update all.
          }
